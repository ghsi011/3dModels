...
1. [`../team-design.md`](../team-design.md): sections 2, 3, 7, 8, and 10.
2. For a solo job only, read and run [`../3d-modeling/SKILL.md`](../3d-modeling/SKILL.md)
   unchanged.

## Checklist

1. Create the project folder and `job_state.md`; create/update the Print Queue entry.
...
5. Dispatch the metrologist to create `dimensions.md`; gate on complete datum/provenance,
   confidence grades, and resolved blockers.
...
8. Dispatch candidate designer(s) against the sheet, accepted reference, and print plan.
   Only CadQuery candidates may run in parallel. Serialize all FreeCAD work through one
   instance.
9. Dispatch a fresh verifier that was never a designer. A `REJECT` returns to
